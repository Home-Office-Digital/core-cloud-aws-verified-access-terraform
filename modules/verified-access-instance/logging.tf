...
# NOSONAR: Safe here. This bucket is the server access log destination (S3 cannot self-log) and HTTPS-only is enforced by aws_s3_bucket_policy.va_access_logs_https_only.
...
# NOSONAR: HTTPS-only is enforced by aws_s3_bucket_policy.va_log_bucket_https_only.
