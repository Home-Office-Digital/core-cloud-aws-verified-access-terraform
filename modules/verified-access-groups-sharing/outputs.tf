...
locals {
  ...
  # against the broader [A-Za-z0-9] alphabet so the parser doesn't reject
  # future ID-format changes or non-hex mock ARNs (e.g. vagr-MOCK used in
  # Terragrunt mock_outputs) in plan-time validation.
  verified_access_group_ids = {
    for name, arn in var.verified_access_group_arns :
    name => regex("vagr-[A-Za-z0-9]+$", arn)
  }
